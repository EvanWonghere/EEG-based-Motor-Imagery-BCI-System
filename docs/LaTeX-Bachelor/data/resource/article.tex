\section{引言}
	本模板按照《山东师范大学学士学位论文模板-理工类（2026）》制作。
...
\section{论文打印与装订要求}
	...
	论文统一用A4纸标准输出，单面打印。要求纸的四周留足空白边缘，上边距2.5cm、下边距2cm、左边距2.5cm、右边距2cm。

\section{一级标题}
	一级标题：黑体（西文TNR）三号加粗，居中，段前24磅、段后18磅，单倍行距。
\subsection{二级标题}
	二级标题：黑体（西文TNR）小三号加粗，左对齐（靠页面最左边），段前24磅、段后6磅，单倍行距。
\subsubsection{三级标题}
	三级标题：黑体（西文TNR）四号加粗，左对齐（靠页面最左边），段前12磅、段后6磅。正文内容使用小四号宋体（西文TNR），两端对齐，首行缩进2字符，固定行距25磅。

	实际写作时：**一级标题**（如“1 引言”）保持居中；**二级及以下标题**（如“1.1 ×××”“1.1.1 ×××”）全部左对齐，从页面最左边开始。

	The font of English text is Times New Roman. 


\section{页眉与页脚}
	每页要有页眉，其上居中打印“山东师范大学学士学位论文”字样（宋体五号）；页码标注在页面底端居中（Times New Roman 五号）。
...
\subsection{图片}
	...
	\begin{figure}[htbp]
		...
		\bicaption{示例图片\label{test-pic1}}{Example figure}
	\end{figure}

	当一个图由两个或两个以上分图组成时，各分图分别以(a)、(b)、(c)…作为图序，并须有分图名，分图名直接列在各自分图的正下方，主图名列在所有分图的下方正中。示例如图\ref{test-subfig} 所示。\par

	\begin{figure}[htbp]
		\centering
		\subfloat[分图一：中文分图名\label{test-subfig-a}]{
			\includegraphics[width=0.45\textwidth]{test.png}
		}
		\hfill
		\subfloat[分图二：中文分图名\label{test-subfig-b}]{
			\includegraphics[width=0.45\textwidth]{test.png}
		}
		\bicaption{示例分图：主图名\label{test-subfig}}{Example subfigures: main caption}
	\end{figure}
	\FloatBarrier
...
\subsection{表格}
	...
	\begin{table}[htbp]
		\centering
		\bicaption{测试表格\label{test-table}}{Example table}
		\begin{tabular}{ccccc}
			\toprule
